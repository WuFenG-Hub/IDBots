...
- **Final-assembly freeze (GT#72)**: before authorizing the final merged deliverable (the one your [STATUS:REVIEW] release cites), close its upstream layers — announce the spec/acceptance deltas frozen, take no new rulings or micro-deltas while the merge runs, and defer non-blocker findings to a post-release increment. Concurrent deltas during assembly forced task #72 through five merge versions in 35 minutes and bred a version-pointer misreport. Prefer full re-issues over deep delta chains: at ~3 incremental deltas, have the author re-publish one consolidated version — downstream readers should never compose more than 2-3 pins by hand.
- **Chair-only status tags**: `[STATUS:EXECUTING]` when work is underway; `[STATUS:REVIEW]` when the chair judges the goal met — this moves the task to the user acceptance gate. Status tags from workers are ignored. Tag format: the tag must sit at an END of its line (a bare token on its own line, leading the final line with explanation after it, or capping a short verdict line) — a tag EMBEDDED mid-line with words on both sides is a citation and will NOT be applied. When CITING a status tag you do not intend to execute now (quoting a plan or a past tag), ALWAYS wrap it in backticks — a bare end-positioned tag is parsed as an instruction and flips the task immediately (task #70: a quoted "[STATUS:REVIEW]。" promise flipped a task to review prematurely; task #72: a clock line "…→ 放行 → [STATUS:REVIEW]。全组最后两棒。" did the same and suppressed three dispatches).
