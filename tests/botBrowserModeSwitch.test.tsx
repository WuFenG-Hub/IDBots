...
test('BotBrowserModeSwitch renders a full-width sidebar segmented control', () => {
  ...
  assert.ok(markup.indexOf('Bot Internet') < markup.indexOf('Bot Home'));
});

test('BotBrowserModeSwitch exposes pressed state with the IDBots filled primary style', () => {
  const markup = renderSwitch('browser');

  assert.match(markup, />Bot Internet<\/button>/);
  assert.match(markup, /aria-pressed="true"[^>]*class="[^"]*\bbtn-idchat-primary-filled\b[^"]*\bstill\b[^"]*"[^>]*>Bot Internet<\/button>/);
  assert.match(markup, /aria-pressed="false"[^>]*>Bot Home<\/button>/);
  assert.doesNotMatch(markup, /aria-pressed="true"[^>]*class="[^"]*\bbg-claude-accentMuted\b[^"]*"[^>]*>Bot Internet<\/button>/);
});
