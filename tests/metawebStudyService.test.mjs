...
// ---------------------------------------------------------------------------
// Legacy qa-surf rows (kind 'qa-surf', feat/metaweb-qa) — RETIRED by MetaWeb
// surf (feat/metaweb-surf): the schema migration retires them on first launch
// and no code path can enqueue or schedule one again. What remains here is the
// disableQaSurfJob cleanup path the pre-migration tool aliases still call.
// ---------------------------------------------------------------------------

test('disableQaSurfJob retires a legacy qa-surf row (pre-migration cleanup path)', () => {
  const { service, store } = setup();
  assert.equal(service.disableQaSurfJob(7), false, 'nothing active yet');
  // A row inserted AFTER construction is untouched by the constructor's
  // migration — this emulates the pre-migration databases the alias tools
  // still clean up.
  store.insert({
    id: 'legacy-qa-surf-1',
    metabotId: 7,
  ...
  const after = store.getById('legacy-qa-surf-1');
  assert.equal(after.status, 'done');
