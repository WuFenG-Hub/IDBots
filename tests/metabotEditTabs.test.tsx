...
test('tab field and sync group mappings pin each editable field to its tab', () => {
  ...
  assert.deepEqual(EDIT_TAB_FIELDS.persona, ['position', 'role', 'soul', 'goal']);
  ...
  // Worker position is a local-only orchestration dimension and must stay off-chain.
  assert.ok(!allSyncGroups.includes('position'), 'position must stay out of the chain sync groups');
