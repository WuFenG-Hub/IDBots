...
import { normalizeA2AByeCooldownMs, normalizeA2AMaxIncomingTurns } from './services/a2aChatLimits';
import type {
  Metabot,
  MetabotInsert,
  MetabotUpdate,
    ...
    if (useBlob) {
      this.db.run(
        `UPDATE metabots SET
          wallet_id = ?, mvc_address = ?, btc_address = ?, doge_address = ?, public_key = ?, chat_public_key = ?, chat_public_key_pin_id = ?,
          name = ?, avatar_blob = ?, enabled = ?, metaid = ?, globalmetaid = ?, metabot_info_pinid = ?, metabot_type = ?, created_by = ?,
          role = ?, soul = ?, goal = ?, bio = ?, background = ?, boss_id = ?, boss_global_metaid = ?, owner_binding_pinid = ?, llm_id = ?, fallback_llm_id = ?, tools = ?, skills = ?, allow_chat_skills = ?, a2a_max_incoming_turns = ?, a2a_bye_cooldown_ms = ?, homepage = ?, updated_at = ?
        WHERE id = ?`,
        [
