...
export function buildTrackedTaskClosureAgentTools(deps: {
  ...
  const acknowledgeCardClosure = tool(
    ...
    'Mark one card conclusion as executed and file the receipt on the SAME card. Required for every item returned by list_pending_card_closures — a conclusion processed without a receipt leaves the card in a state nobody can audit. The receipt must state how it was handled plus its evidence, or use the explicit no-op marker when nothing had to be done. If the conclusion asks for a destructive action (delete / transfer / publish / any irreversible change), it must first go through the existing safety confirmation and the resulting reference must be passed as confirmationRef; without it this call is refused and no state is written. v1.5 self-closure: a TERMINAL card that YOU (the Twin) initiated which carries no closing conclusion can be closed through this same tool — the receipt becomes the card conclusion and is written off in the same statement, so it never appears in the pending queue. A card the owner initiated still refuses without a conclusion: never write a conclusion on the owner\'s behalf.',
    {
