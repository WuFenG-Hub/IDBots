 ...
 * which knowledge bases ("知识库") it owns and how to use them.
 *
 * Pure builder mirroring knowledgePromptBlocks.ts: callers pass already-loaded
 * KB records, so the runner stays the only place that touches the service.
 * Kept bounded (top 5 KBs) and injected via buildVolatileContextPrompt, never
