...
export function buildSurfSessionPrompt(context: SurfSessionContext): string {
  ...
  return [
    ...
    '',
    memoryOff
      ? `1. REVIEW the digest above. Judge by title/summary against your persona and shortlist the pins you genuinely care about (at most ~${SURF_DEEP_READ_GUIDANCE} deep reads). Then read the shortlist with read_metaweb_pins_batch calls of ~10-12 pinIds — the whole batch result is budget-capped, so bigger batches get bodies trimmed or omitted (each entry says so); an omitted body did NOT count as a read, follow up with read_metaweb_pin on those. Memory is OFF tonight — nothing can be saved; just read and judge.`
      : `1. REVIEW the digest above. Judge by title/summary against your persona and shortlist the pins you genuinely care about (at most ~${SURF_DEEP_READ_GUIDANCE} deep reads). Then read the shortlist with read_metaweb_pins_batch calls of ~10-12 pinIds — the whole batch result is budget-capped, so bigger batches get bodies trimmed or omitted (each entry says so); an omitted body did NOT count as a read, follow up with read_metaweb_pin on those. Use a single read_metaweb_pin only to continue after a batch entry came back truncated (server cap or budget trim — payload is never truncated) or for a pin discovered via search later. For each pin worth keeping long-term: knowledge_base_add_document with sourceType 'metaweb', the pinId, its title, and the full body (payload field if truncated) into a topical knowledge base from your <knowledge_bases> list (default one otherwise). Distill durable facts into knowledge_upsert, and a repeatable workflow into procedure_save.`,
    memoryOff
