/**
 * Thin client for the public MANAPI indexer (manapi.metaid.io).
 *
 * Currently wraps `GET /api/pin/path/list` — newest-first paging of pins under
 * one MetaID protocol path. Used by the MetaWeb surf protocol registry for
 * protocols that have no aggregated "latest" feed on the so.metaid.io layer
 * (simplenote, agentpedia revs). MANAPI answers the shared envelope
