...
export function createGroupTaskDaemonLoop(deps: GroupTaskDaemonDeps): GroupTaskDaemonLoop {
  ...
  const notifySourceSessionMilestone = (
    ...
    try {
      // EP33 / P3①: carry the EVENT timestamp inside the notice — the origin
      // session consumes queued relays lazily (it may wake hours later), and
      // without the stamp the reading Twin mistakes an on-time delivery for a
      // delayed one.
      const eventAt = new Date(now());
      const pad = (v: number) => String(v).padStart(2, '0');
      const stamped = `${message}\n(event at ${eventAt.getFullYear()}-${pad(eventAt.getMonth() + 1)}-${pad(eventAt.getDate())} ${pad(eventAt.getHours())}:${pad(eventAt.getMinutes())} local)`;
  ...
  const maybeRunChairPlanningTurn = async (
    ...
    // EP33 / P2: hoist the already-dispatched gate BEFORE the LLM call. The
    // owner's Twin may have opened the task manually (voting kickoff etc.) —
    // when chair-authored worker-mentioning speech already exists, a full
    // bootstrap plan would duplicate it (EP33: welcome + candidate pool
    // re-posted AFTER the votes landed, costing ~9% of the transcript in
    // clarifications).
    if (chairAlreadyDispatched(sqlite.getDatabase(), task, members, botsById)) {
      // The trigger only runs this bootstrap for planning tasks; with an
      // active chair, run ONE minimal turn whose
      // only legitimate output is the missing lifecycle transition (typically
      // [STATUS:EXECUTING]) — never a repeat of any existing content.
      try {
        ...
        const brain = metabotBrainOptions(bot);
        const minimalReply = (await performChatWithTimeout(
          systemPromptParts.systemPrompt,
          minimalDirective,
          brain.llmId ?? undefined,
          { llmProvider: brain.llmProvider, fallbackLlmId: brain.fallbackLlmId, fallbackLlmProvider: brain.fallbackLlmProvider, effort: brain.effort, fallbackEffort: brain.fallbackEffort, thinking: 'enabled' },
        )).trim();
        if (minimalReply && !NO_REPLY_PATTERN.test(minimalReply)) {
          await postGroupMessage(task.id, bot.id, minimalReply);
        }
        sqlite.set(plannedKey, '1');
        emitLog(
          `[GroupTaskDaemon] Task ${task.id}: planning bootstrap completed minimally ` +
          `(${minimalReply && !NO_REPLY_PATTERN.test(minimalReply) ? 'missing step posted' : 'nothing missing — [NO_REPLY]'})`,
        );
        return;
      } catch (error) {
        ...
        emitLog(
          `[GroupTaskDaemon] Task ${task.id}: minimal planning turn failed (attempt ${nextAttempts}/${MAX_CHAIR_PLAN_ATTEMPTS}): ` +
          `${error instanceof Error ? error.message : String(error)}`,
        );
        return;
      }
  ...
  const processSupervisorSignals = async (
    ...
    // EP33 / P1②: REVIEW no longer defers the supervisor turn. The chair may
    // speak in review (it answers the owner there), and a supervisor nudge IS
    // the owner representative's message — same authority class as an owner
    // message. The chair verifies and either reopens rework
    // ([STATUS:EXECUTING] is a legal review→executing transition it owns) or
    // answers why not. An OPEN CHECKPOINT still defers: the owner is
    // mid-decision and nudging around them would create two dialogs.
    if (store.getOpenCheckpoint(task.id) != null) {
      return; // human checkpoint active — defer the chair turn until it clears
    }
    ...
    const directive = [
      ...
      // EP33 / P1②: in review the nudge is typically a defect found during
      // acceptance — reopening rework IS the legitimate lifecycle move.
      ...(task.status === 'review'
        ? [
