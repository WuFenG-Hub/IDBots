...
export function createGroupTaskDaemonLoop(deps: GroupTaskDaemonDeps): GroupTaskDaemonLoop {
  ...
  const buildOwnerReportDirective = (store: GroupTaskStore, task: GroupTask): string => {
    ...
    return [
      ...
      '- FORMAT REQUIREMENT: the FIRST line of your report must be exactly 【结论】<one-line verdict, ≤80 chars> — accept & close, or rework and of what. The host extracts that line verbatim as the single conclusion string reused by the Tasks acceptance-card headline, the group summary, and the origin-session notice; keep it a verdict, never a question.',
      ...
      '- Recommend an action (accept & close, or request rework — of what). The owner only needs to confirm acceptance on the acceptance card in the Tasks UI or send the task back for rework; never end with an open-ended "what would you like to do next?".',
      '',
        ...
        try {
          ...
          if (beforeStatus && updated.status !== beforeStatus) {
            ...
            if (updated.status === 'review') {
              ...
              // the owner private report (above) and the R2 source-session
