 ...
 * night.
 *
 * Job kind 'qa-surf' (on-chain Q&A surfing, feat/metaweb-qa) is RETIRED:
 * MetaWeb surf (feat/metaweb-surf) superseded it — the schema migration
 * retires legacy rows on first launch, and the nightly Q&A toggle now maps
 * to the bot's surf-before-dream setting via the legacy tool aliases. The
 * kind remains in the record type (and the status listing) for legacy rows
 * only; no code path can enqueue or schedule one again.
 */
...
export class MetawebStudyService {
  ...
  /**
   * Legacy cleanup path (kept for the pre-migration tool aliases): retire the
   * bot's active Q&A surfing job row. Returns true when an active job was
   * disabled, false when there was nothing to stop — which is every call on
   * a post-migration database, since no code path can enqueue one again.
   */
