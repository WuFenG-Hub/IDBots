 ...
 * (MetaSo spec §5 / IDBots spec §3, docs/metaid_protocols/metaprotocol-registry-agent-tools.md):
 *
 ...
 *
 * Same envelope conventions as the other aggregation APIs ({code, data,
 * message}, HTTP always 200, business error codes 40000/40400/50000).
 *
 * Read-only MANAPI fallback (manapi.metaid.io): when the MetaSo projection is
 * unreachable, the agent tools degrade to GET /pin/path/list?path=/protocols/metaprotocol
 * plus client-side contentSummary parsing, and to the source pin's
 * modify_history for version chains. The fallback lives in this service so the
 * tool layer only orchestrates; nothing here ever writes.
 */
