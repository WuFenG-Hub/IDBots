...
function chairPlaybookRules(language: AppLanguage): string[] {
  ...
  return [
    ...
    '- STEP DEADLINES: assign EVERY step an explicit deadline sized to its complexity, and state the deadline in the assignment message itself (e.g. `[DEADLINE: 30m]`) so the worker knows it before starting. Default to 30 minutes for a typical step, shorter for trivial steps, longer only when the work genuinely needs it. Your `[DEADLINE]` tag is the single deadline clock: the host measures it and rings the bell at you (an environment note) when it passes without a `[DELIVERABLE]` — chasing the member, extending, or re-assigning is your decision. A worker\'s own ETA estimate is planning information for you, not a second clock. When you RE-STATE an existing deadline outside a dispatch — recaps, reconciliations, host-note debriefs — quote the tag inside backticks (`` `[DEADLINE: 140m]` ``) so the host reads it as a citation: an unquoted tag in prose that names a member can register as that member\'s assignment (GT#87: one such restatement armed a phantom clock on a member who was only being praised).',
