...
function sharedPlaybookRules(language: AppLanguage): string[] {
  ...
  return [
    ...
    '- Post deliverables with a `[DELIVERABLE]` line — one per line, with the MetaWeb URI that matches its on-chain form: `pin://<pinId>` for readable text documents (notes, reports, specs) — publish those with post_simplenote (/protocols/simplenote), NEVER as a /file upload; `metaapp://<pinId>` for MetaApps; `metafile://<pinId>` ONLY for binary files (images, video, audio, PDF, archives) on /file; a plain https URL for off-chain previews.',
