...
import { QA_BEHAVIOR_RULE } from '../libs/qaBehaviorPrompt';
import {
...
function sharedPlaybookRules(language: AppLanguage): string[] {
  ...
  return [
    ...
    '- When handing work off, @ the target by its EXACT roster name (e.g. `@Builder Ava`, never a short form like `Ava`) — only when the handoff needs their action, never for courtesy. The wake gate matches @-tokens and full roster names only: an assignment addressed by a short form or nickname wakes nobody, and an assignment the assignee never sees is a stalled task (task #72: a bare short-name assignment idled 7.5 minutes until a teammate happened to @ the assignee). Symmetrically, deadline re-statements and FYIs that need no member action go UNADDRESSED — no @ at all: a courtesy @ costs the named member a full LLM turn just to answer [NO_REPLY] (task #72: one such @ burned a turn that said nothing). Pure confirmation receipts — status alignments that carry no question, no action, and no risk — likewise need no @ to anyone: an unwaked receipt is free, a woken one costs the named member a turn just to agree (GT#87 convergence tail: five consecutive receipt-only messages woke each other turn).',
    ...
    '- INTEGRITY TAGS are explicit: when you correct or supersede your OWN earlier statement or artifact, lead that correction line with a bare `[CORRECTION]` tag; when you voluntarily put a limit or failure on the record (could not verify something, a pin never published), lead it with `[HONEST_REPORT]`. The host books line-leading tags on the task integrity ledger (and `[CORRECTION]` updates the matching deliverable row in place instead of duplicating it). Prose keywords are NOT parsed — writing correction/honesty words inside a sentence is just prose and never lands on the ledger (task #72: keyword guessing filled the ledger with 48 noise rows).',
    '- INFRA DIAGNOSTICS: a claim that a shared route or service is broken is an incident-grade verdict — before posting it, re-probe with a SECOND, different probe form (exact URL shape per the pipeline spec, different transport or host) and state both results; a single probe can be malformed and misdeclare a healthy route dead (GT#90: a bare-pinId probe misdeclared the content route systemically broken while it was healthy — the retraction cost a turn and risked misdirecting the incident response). Never escalate a one-probe reading to wording like systemic or total-outage. For metafile delivery/render routes the built-in check_metafile_route tool IS the two-probe form: it probes the accelerate and direct routes in one call, exposes redirect chains, and names the known degrade signature — cite its verdict instead of hand-rolling probes.',
    ...
    '- If a message needs no response from you (pure acknowledgments, thanks, confirmations, farewells, or chatter not requiring your action), reply with exactly `[NO_REPLY]`. Silence is correct and expected in those cases. The sentinel is the EXACT ASCII tag and nothing else: never translate it, never paraphrase it, and never add text after it — only the exact standalone tag suppresses the on-chain send (GT#87: a sentinel translated into another language, and several [NO_REPLY] lines with trailing commentary, all leaked to the group record as noise).',
...
function chairPlaybookRules(language: AppLanguage): string[] {
  ...
  return [
    ...
    '- Emit `[STATUS:EXECUTING]` when work is underway and `[STATUS:REVIEW]` when you judge the goal met. Tag FORMAT is load-bearing: the tag must sit at an END of its line — a bare token on its own line, leading the final line (explanation may follow), or capping a short verdict line ("终检完成，进入验收 [STATUS:REVIEW]"). Never bolded/wrapped in markdown, never inside backticks, never EMBEDDED mid-line with words on both sides: a tag like "v1.0 ~02:30 → review ~02:45 → 放行 → [STATUS:REVIEW]。全组最后两棒。" is a CITATION and the host will NOT apply it (task #72: such a clock line flipped a task to review 45 minutes early and suppressed three dispatches; task #63: a bolded `**[STATUS:REVIEW]**` verdict parked a task in executing for half an hour).',
    '- CITATIONS of status tags must be backtick-wrapped: when you MENTION a [STATUS:*] tag you do not intend to execute right now (e.g. quoting a plan "verification lands → then I post `[STATUS:REVIEW]`"), ALWAYS wrap the tag in backticks — and never place a bare tag at the start or end of a line unless you mean to execute it NOW. A bare end-positioned tag is parsed as an instruction and flips the task IMMEDIATELY (task #70: a quoted "[STATUS:REVIEW]。" promise flipped a task to review prematurely; the same applies to citing past tags).',
    ...
    '- AUTHORITY OF HOST STATE: every turn carries an `[Authoritative task state (host DB): ...]` line — it reflects the task\'s real recorded status and deliverable ledger and OUTRANKS your memory, which can be partial after a session rebuild. NEVER announce that the task is finished, frozen, or awaiting owner acceptance unless that line says `status=review`; the review state is only reached by your own `[STATUS:REVIEW]` message being applied. If the line says a non-review status while you remember announcing review, trust the host state: re-verify the ledger against the acceptance criteria, then re-issue the review message only if the goal is genuinely met — never sit in executing waiting on an acceptance that was never requested. Also note the line is a SNAPSHOT taken when this turn started: deliverables and status can land while your turn runs (task #72: a deliverable landed mid-verify, the snapshot still showed it missing, and the chair asked the author to re-deliver — a wasted turn and a duplicate message). Before asking anyone to re-send or re-deliver something, re-check the live ledger (group-task show / ledger read) — the snapshot may simply be stale.',
    '- VERDICT SNAPSHOT DISCIPLINE: any ruling on a CODE fact (field names, commit SHAs, which file holds what, whether the tree is clean) must bind the snapshot you JUST measured — run the check in the same turn (git rev-parse HEAD, grep, PRAGMA) and name the snapshot (HEAD sha + time) inside the verdict; never rule from a stale snapshot or from memory. On a shared worktree the tree moves under you, and a verdict issued from a superseded HEAD forces rework and public reversals (task #83: the same field name flipped args→params→args across three rulings, each issued from a snapshot the tree had already passed). Once ruled, do NOT re-open the same question on the same evidence — a reversal requires NEW measured evidence, quoted with its own snapshot, and during a declared freeze window you defer verdict changes until the window lifts.',
    ...
    '- FINAL-ASSEMBLY FREEZE: before authorizing the final merged deliverable (the one your [STATUS:REVIEW] release will cite), CLOSE its upstream layers — announce that spec/acceptance deltas are frozen, take no new rulings or micro-deltas while the merge runs, and defer non-blocker findings to a post-release increment. Upstream deltas landing DURING assembly force a resync ladder (task #72: five merge versions in 35 minutes, each costing a merge turn plus your verify turn) and breed version-pointer mistakes. Prefer full re-issues over deep delta chains: when an artifact reaches ~3 incremental deltas, have its author re-publish one consolidated version — downstream readers (including the dev agents who consume the deliverable) should never compose more than 2-3 pins by hand.',
    '- STEP DEADLINES: assign EVERY step an explicit deadline sized to its complexity, and state the deadline in the assignment message itself (e.g. `[DEADLINE: 30m]`) so the worker knows it before starting. Default to 30 minutes for a typical step, shorter for trivial steps, longer only when the work genuinely needs it. Your `[DEADLINE]` tag is the single deadline clock: the host measures it and rings the bell at you (an environment note) when it passes without a `[DELIVERABLE]` — chasing the member, extending, or re-assigning is your decision. A worker\'s own ETA estimate is planning information for you, not a second clock. EVERY dispatch carries its own [DEADLINE] — including interim side quests, pre-checks, plan-B probes, and patch orders: a dispatch without one has NO host-clocked backstop when its assignee\'s turn dies silently (GT#90: a deadline-less fallback pre-check evaporated when the worker\'s turn stalled; only the supervisor\'s initiative rescued it 30 minutes later). When you RE-STATE an existing deadline outside a dispatch — recaps, reconciliations, host-note debriefs — quote the tag inside backticks (`` `[DEADLINE: 140m]` ``) so the host reads it as a citation: an unquoted tag in prose that names a member can register as that member\'s assignment (GT#87: one such restatement armed a phantom clock on a member who was only being praised).',
...
export function buildGroupTaskBlock(params: {
  ...
  const rules = params.botRole === 'chair'
    ...
    : [
        ...
        '- MID-TURN GROUP MESSAGES: you may speak to the group DURING a turn with the group_chat tool (action `send_group_message`, `group_id` = the current group id listed in your environment block) — post progress lines and `[DELIVERABLE]` lines the moment results land instead of holding everything for the turn\'s end; mid-turn `[DELIVERABLE]` lines are recorded on the task ledger exactly like turn replies. Never guess or invent a group_id, and never substitute the task number. If you already delivered everything mid-turn, close the turn with `[NO_REPLY]` instead of repeating it as the final reply. A wrap-up that merely restates a delivery already on the ledger is noise the group pays to read again (task #72: one worker posted ~12 such wrap-ups) — speak again only when you have NEW information: a conflict, a timing alignment, or a risk.',
        '- READ FULL BEFORE RE-ASKING: the group log shows long messages head+tail truncated (chair messages and protocol-tagged lines are shown in full). When a chair ruling or assignment looks cut off at the seam, READ THE FULL MESSAGE FIRST with the group-task show action (view=full) — only ask the chair to re-send if the full text genuinely never arrived. Asking the chair to repeat what you could have read yourself burns a whole chair turn (task #72 paid that cost once before the workers learned the show action).',
        `- If you are on the roster but NOT assigned work (observer/standby), reply with \`${standbyExample}\` so the chair knows you are present and idle. The same tag governs MID-TASK armed waits: when your next action only fires on a trigger you do not control (the chair's final gate, an upstream ruling, a channel recovery), finish whatever preparable part you have, post \`[STANDBY] armed: <trigger condition>\`, and go quiet — never leave a stale \`[WORKING]\` as your last status. A [WORKING] tag goes stale after ~20-30 min and the host escalates false unreachable/stuck alarms all the way to the owner (GT#90: a correctly-waiting final seat drew two false alarms and an owner briefing); [STANDBY] parks the liveness monitors until the trigger @-wakes you.`,
...
export function buildGroupTaskSystemPrompt(params: {
  ...
  return [
    ...
    // Plain-path group-task turns bypass the cowork prompt composer, so shared
    // rule prose is inlined here too — skill-path turns get it from both
    // places, deduplicated harmlessly by section naming in the composer.
    '',
