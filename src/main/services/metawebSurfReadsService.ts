 ...
 * the same so.metaid.io aggregation backend (LIVE in production):
 *
 ...
 *
 * Same conventions as metawebPinService: {code, data, message} envelope,
 * HTTP always 200, business error codes 40000/40400, AbortController timeout
 * with a mapped message. Cursors are OPAQUE server tokens — stored and
 * forwarded verbatim, never parsed client-side.
 */

import {
  normalizePin,
  type MetawebPin,
} from './metawebPinService';
