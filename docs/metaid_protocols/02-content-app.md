...
**MetaFile URI best practice**: Protocol parsers may accept both `metafile://<pinId>` and `metafile://<pinId>.<ext>`. Official apps and skills should write extension-bearing URIs when assembling new payloads, deriving the extension from the `/file` pin `contentType` or the local filename. This lets clients infer file type without an extra file-pin lookup.
...
```json5
{
  ...
  "skillDocument": "metafile://document.md",
