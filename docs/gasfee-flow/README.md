...
| 2026-08-10 | Client Phase C (fee-rate unification, 32 call sites) and Phase D (trafficAccountService + IPC + local journal + trafficAccount injection into sponsor pre for pins and uploads) committed. Spec errata recorded (backend-spec §12, implementation-authoritative). Backend fixing the 2 must-fix items. |
| 2026-08-10 | Client Phase E committed (91ce02a2): Settings > Traffic tab — mode toggle + auto-bind, balance card, recharge flow with mock payment, traffic center (per-bot daily usage, ledger). **Client Phases A–E all done.** Remaining before real payment: testnet joint integration (M1.7) and manual on-device QA of the panel. |
| 2026-08-10 | Backend must-fix re-review: PASS (both closed, tests green, no new blockers). GlobalMetaID derivation verified identical on both sides (fixture 15St4DUg… → idq1xrzc2…, exact match) — account creation is not an integration blocker. Ready for testnet integration (M1.7). |
| 2026-08-10 | **M1.7 done — joint integration PASSED.** Backend instance live (mainnet chain, isolated keys). Full closed loop verified via `scripts/traffic-e2e/`: account create (server-derived accountId == local GlobalMetaID), bindings, mock recharge (idempotent), real sponsored pin txid 479a8c76… with balance delta == txSize (510 bytes, quota untouched), ledger/usage/admin consistency. Extended acceptance: all Spec §9 criteria PASS live (concurrency 20 pres conserved, expiry release, TRAFFIC_INSUFFICIENT, legacy quota regression, dynamic fee rate with bytes-semantics billing, plan CRUD, idempotent admin grants). Fixed one real bug: address/info `status` omitempty (cff4c060). Phase 1 complete. |
