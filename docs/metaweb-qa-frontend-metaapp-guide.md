...
For frontend application developers, 2026-09-07. Everything described here is live in production: the protocols are on-chain, the MetaSo Q&A index and APIs are deployed at `https://so.metaid.io`, and a minimal reference viewer ships inside IDBots (the bundled `qanda` MetaApp). This guide is self-contained — you should not need any other IDBots-internal document to build a human-facing Q&A MetaApp — but pointers to the full specs are at the end.
...
- **PayComment** `/protocols/paycomment`: `{ "content", "contentType", "commentTo": "<pinId>" }` — comments on questions or answers. `commentCount` is exposed by the APIs; **reading comment content is currently a gap — see §6 G1.**
...
- **Versions**: `modify` updates fields in place; `revoke` hides the question and thereby its answers (detail endpoints return 40400 for hidden questions). List items carry `currentPinId`; cite the stable `pinId`.
...
Returns question items (§3.5) plus a `score`. This is the search box of your app, and also the "has anyone asked this before" check.

### 3.2 `GET /api/qa/questions` — feeds

| Param | Notes |
|---|---|
| `tags` / `minAnswers` / `maxAnswers` | `maxAnswers=0` = the **unanswered queue** |
...
Question item (search/feed/detail):
...
`topAnswer` is `null` while unanswered. Answer items (detail/answers endpoints) carry `questionPinId`, `summary` (~200 runes), `tags`, `publisher`, `createdAt`, `isMempool`, `likeCount`/`dislikeCount`/`commentCount`, and the derived `score`.

**List surfaces return summaries only** — never full bodies. That is deliberate; fetch full content per pin via §3.6.

### 3.6 `GET /api/metaweb/pin/:pinId` — full body of any pin
...
4. **Attachments** come resolved from §3.6 — never construct `metafile://` URLs yourself.
5. **Links inside IDBots' Bot Browser**: `pin://<pinId>` opens the built-in viewer (question pins route to the Q&A page), `metaid://<globalMetaId>` opens the identity's page — prefer these over Web2 viewer URLs. A standalone app outside IDBots can deep-link back with its own routes.
6. **Poll cheaply or not at all**: feeds answer freshness needs; there is no websocket/SSE (see §6 G6 if you need one).
...
## 6. Gap analysis — is the current API surface enough for a human-facing app?

Mostly yes: browse, search, question pages, ranked answers, author-filtered answers, unanswered queue, hot feed all exist. Building a real product surface still leaves these gaps, ordered by impact:

- **G1 (P1) — Comment threads are unreadable.** `commentCount` exists, but comment **content** for Q&A pins is not served by any documented endpoint: the social comments API (`GET /api/social/post/:pinId/comments`) is simplebuzz-only in v1, and the Q&A index deliberately stores no comment bodies. ZhiHu-style comment threads cannot be built. Proposed: `GET /api/qa/pins/:pinId/comments` (paged, same envelope) — or extend the social comments endpoint to accept Q&A target pins.
- **G2 (P2) — Author pages cannot list answers.** "All questions by user X" works (`search?publisher=`), but "all answers by user X across questions" has no endpoint (publisher filtering exists only per question). Proposed: `GET /api/qa/answers?publisher=&cursor=` reusing the existing answer index.
...
- Backend contract (API details beyond this summary): metaso-p2p `docs/specs/2026-09-07-metaweb-qa-api.md`
